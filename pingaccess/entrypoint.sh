...
if test "$1" = 'start-server' ; then
	...
	if ! test -d ${OUT_DIR}/instance ; then	
		# if not, then we deploy the bits to /opt/out
		cp -af /opt/server/ ${OUT_DIR}/instance

		apply_server_profile
	fi

	cd ${OUT_DIR}/instance/log
