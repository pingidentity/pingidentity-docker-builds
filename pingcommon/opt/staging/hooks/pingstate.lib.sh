...
add_state_info ()
{
    ...
    if test "${_sourceInfo}" = "environment_variables"
    then
        env | sort | grep -v "^HOST_NAME="> "${_stateDir}/${_hashFile}"
    elif test -f "${_sourceInfo}"
    then
        cp "${_sourceInfo}" "${_stateDir}/${_hashFile}.file"
    elif test -d "${_sourceInfo}"
    then
...
compare_state_info()
{
    ...
    if test ! -d "${STATE_DIR}/current" || test -z "${_prevState}" || test ! -d "${STATE_DIR}/${_prevState}"
    then
        ...
        while IFS= read -r _fileToDiff
        do
            if test ! -f "${STATE_DIR}/${_prevState}/${_fileToDiff}"
            then
                echo "New file found in current container - ${STATE_DIR}/${_prevState}/${_fileToDiff}"
                continue
            fi
            if test ! -f "${STATE_DIR}/${_currState}/${_fileToDiff}"
            then
