...
set_java_home_and_args() {
  ...
  if test -z "${PRIVATE_UNBOUNDID_JAVA_BIN}" ; then
    if test -z "${PRIVATE_UNBOUNDID_JAVA_HOME}" ;then
...
set_environment_vars() {
  ...
  if test -z "${UNBOUNDID_ENV_VARS_SET}" ; then
    ...
    if test -z "${UNBOUNDID_PATH_OVERRIDE}" ; then
      PATH=/bin:/usr/bin
    else
    ...
    # of 65535.
    if test -z "${NUM_FILE_DESCRIPTORS}" ; then 
      if test -s "${INSTANCE_ROOT}/config/num-file-descriptors" ; then
    ...
    if test "$(uname)" != "Darwin" ; then
      ...
      ACTUAL_FDS=`ulimit -n`
      if test "${NUM_FILE_DESCRIPTORS}" -ne "${ACTUAL_FDS}" ; then
    ...
    if test -z "${NUM_USER_PROCESSES}" ; then
      if test -s "${INSTANCE_ROOT}/config/num-user-processes" ; then
        . "${INSTANCE_ROOT}/config/num-user-processes"
      fi

      if test "$(uname)" = "Linux" && test -z "${NUM_USER_PROCESSES}" ; then
        ACTUAL_PROCESSES=`ulimit ${PROCESSES_OPTION}`
        if test "${ACTUAL_PROCESSES}" -lt 16383 2>/dev/null ; then
    ...
    for f in ulimit-num-file-descriptors.out ulimit-num-user-processes.out ; do
      if test -f "${INSTANCE_ROOT}/logs/${f}" ; then
        if test ! -s "${INSTANCE_ROOT}/logs/${f}" ; then
...
set_pg_env() {
  ...
  if test -f "${PG_BIN_TMP}/bin/initdb" ; then
    PG_BIN_DIR="${PG_BIN_TMP}/bin"
    PG_DB_CTRL="${PG_BIN_TMP}/bin/db_ctrl"
  fi

  if test -f "${PG_BIN_TMP}/bin/64/initdb" ; then
...
test -z "${INSTANCE_ROOT}" && INSTANCE_ROOT=$( cd `dirname "${0}"/..`; pwd )
